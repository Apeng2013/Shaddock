	...
	public:
		...
		void BeginSession(const std::string& name, const std::string& filepath = "result.json")
		{
			...
			if (m_CurrentSession)
			{
				...
				{
					SD_CORE_ERROR("Instrumentor::BeginSession('{0}') when session '{1}' already open.", name, m_CurrentSession->Name);
				}
			}
			...
			else
			{
				if (Log::GetCoreLogger())
				{
					SD_CORE_ERROR("Instrumentor could not open results file '{0}'", filepath);
				}
			}
		}
		void EndSession()
